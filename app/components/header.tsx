...
<div className="flex gap-2 items-center xs:gap-1">
  ...
  <div className="flex items-center gap-3 xs:gap-1">
    {loading ? null : (
     <>{ user?
      (<button className="h-12 px-4 rounded-full bg-black  md:h-10 xs:px-3  w-auto text-white xs:w-auto  xs:text-sm  xs:h-8  font-[800] relative" 
  type="button" onMouseEnter={toggleDroppedPopup}  onMouseLeave={toggleDroppedPopup} ref={DroppedRef}>
  {user?.lastName} {user?.firstName}
{isDropped && (
  <div  className={`rounded-xl py-[2px]  px-[0px]   backdrop-blur-lg bg-black  absolute  popup z-30 flex right-0  overflow-hidden top-[48px] w-[160px]     ${isPopupVisible ? '' : 'popup-hidden'}`} >
