...
export async function POST(req: NextRequest) {
  try {
    ...
    // Ensure the book object contains the required 'url' field
    ...
    const isBookInToRead = user.toRead.some((b: { url: string }) => b.url === book.url);
    if (isBookInToRead) {
    ...
    user.currentlyReading = user.currentlyReading.filter((b: { url: string }) => b.url !== book.url);

    // Remove the book from `alreadyRead` if it exists there
    user.alreadyRead = user.alreadyRead.filter((b: { url: string }) => b.url !== book.url);
